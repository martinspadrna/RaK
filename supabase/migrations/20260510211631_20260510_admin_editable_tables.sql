...
do $$ begin
  if not exists (
    select 1 from pg_policies where schemaname = 'public' and tablename = 'machine_settings' and policyname = 'machine_settings_read_all'
  ) then
...
do $$ begin
  if not exists (
    select 1 from pg_policies where schemaname = 'public' and tablename = 'rotation_months' and policyname = 'rotation_months_read_all'
  ) then
...
do $$ begin
  if not exists (
    select 1 from pg_policies where schemaname = 'public' and tablename = 'rotation_entries' and policyname = 'rotation_entries_read_all'
  ) then
...
-- Keep writes restricted for now; admin UI will use a secured path.
;
