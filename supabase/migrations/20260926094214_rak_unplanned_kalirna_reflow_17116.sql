...
begin
  ...
  if v_kind<>'daymod' or v_reason<>'kalirnaOut' then
    raise exception 'Unsupported unplanned change' using errcode='22023';
  end if;
  ...
  if ((p_payload->'months')-v_month_key) is distinct from ((v_current.payload->'months')-v_month_key) then
    raise exception 'Unplanned change may modify only selected month' using errcode='22023';
  end if;
  ...
  if (v_new_month-array['hard','soft','dayMods']::text[])
     is distinct from (v_old_month-array['hard','soft','dayMods']::text[]) then
    raise exception 'Kalirna change may modify only selected assignments and dayMods' using errcode='22023';
  end if;

  foreach v_section in array array['hard','soft'] loop
    v_old_section:=v_old_month->v_section;
    v_new_section:=v_new_month->v_section;
    ...
    if (v_new_section-'rows') is distinct from (v_old_section-'rows') then
      raise exception 'Kalirna change may not modify section metadata' using errcode='22023';
    end if;
    v_old_rows:=v_old_section->'rows';
    v_new_rows:=v_new_section->'rows';
    if jsonb_typeof(v_old_rows)<>'array'
       or jsonb_typeof(v_new_rows)<>'array'
      ...
      for v_idx in 0..v_len-1 loop
        ...
        if not v_allowed then
          if v_new_row is distinct from v_old_row then
            raise exception 'Kalirna change touched a non-selected day' using errcode='22023';
          end if;
        elsif (v_new_row-'cells') is distinct from (v_old_row-'cells') then
          raise exception 'Kalirna change may modify only assignments on selected days' using errcode='22023';
        end if;
      end loop;
    end if;
    select count(distinct row_item.value->>'date') into v_count
    from jsonb_array_elements(v_old_rows) row_item(value)
    where (row_item.value->>'date') in (
      select value from jsonb_array_elements_text(p_allowed_date_labels)
    );
  ...
  if exists (
    select 1
    from (values ('hard'),('soft')) sec(name)
    cross join lateral jsonb_array_elements(v_new_month->sec.name->'rows') r(value)
    cross join lateral jsonb_array_elements(coalesce(r.value->'cells','[]'::jsonb)) c(value)
    where coalesce(r.value->>'date','') in (select value from jsonb_array_elements_text(p_allowed_date_labels))
      and coalesce(c.value #>> '{}','')=v_person
  ) then
    raise exception 'Kalirna person is still assigned to a machine' using errcode='22023';
  end if;
  ...
  if v_new_other is distinct from v_old_other then
    raise exception 'Kalirna change touched unrelated dayMods' using errcode='22023';
  end if;
  ...
  loop
    ...
    if v_section not in ('hard','soft') then
      raise exception 'Invalid Kalirna source section' using errcode='22023';
    end if;
    if coalesce(v_mod->>'cellIndex','') !~ '^[0-9]+$' then
      raise exception 'Invalid Kalirna source cell' using errcode='22023';
    end if;
    ...
    if not exists (
      ...
      raise exception 'Kalirna source cell does not contain selected person' using errcode='22023';
    end if;
  end loop;
