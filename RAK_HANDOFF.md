...
- Současný ověřený development runtime je `1.7.86`. Dokumentační nebo čistě testovací následník bez změny runtime číslo verze nezvyšuje.
...
- Poslední ověřený **funkční** development release: `0eeaf9841ebf375f590451420a9182dafd43033c` (`1.7.86`). Živý HEAD se vždy zjišťuje online; případný následný dokumentační commit nesmí být zaměněn za nový runtime release.
...
- Stabilní development alias nyní ukazuje na `dpl_HBdcUbvP34mxj6mH1hJmdj5BfLoa`, READY / SHA `0eeaf9841ebf375f590451420a9182dafd43033c`. Přesný SHA prošel [Actions #280](https://github.com/martinspadrna/RaK/actions/runs/36017186984) SUCCESS; release job přesunul alias až po celé fail-closed bráně. Ověřený rollback cíl je READY `dpl_8QFJuAL4bwpfsVF6UkrHrxuPzX3c` / předchozí development 1.7.85.
- Release 1.7.86 prošel dvěma kanonickými buildy, celým `npm run check`, zděděnými i novými gate testy, rollback/ZIP důkazy, reálným Chromium online→offline→online testem, třemi PWA benchmarky a živou anonymní TEST HTTP kontrolou. Release evidence `rak-release-evidence-0eeaf9841ebf375f590451420a9182dafd43033c` má artefakt ID `10815820131`.
- Development metadata: viditelná `1.7.86`, technická `1.7.86`, modulová cache `1.7.86`, `package.json` `1.7.86`, SW cache `v1.7.86`, build `v1.7.86-shift-calendars1`. TEST Supabase zůstává `cgshssdjgzzuprlwnabl`; release 1.7.86 nepřidal databázovou migraci a používá existující autentizovanou cestu `rak_admin_save_machine_settings_v2` pro konfiguraci směnových kalendářů. Dřívější account-scoped appearance CAS migrace `rak_account_ui_preferences_cas_17084` zůstává pouze v TEST.
...
**Aktuální online stav k 24. 9. 2026:** produkční předání 1.7.83 včetně schválené bezpečnostní fáze B je dokončeno a produkce zůstává beze změny. Nejnovější TEST runtime je 1.7.86 na funkčním SHA `0eeaf9841ebf375f590451420a9182dafd43033c`, [Actions #280](https://github.com/martinspadrna/RaK/actions/runs/36017186984) SUCCESS a Vercel `dpl_HBdcUbvP34mxj6mH1hJmdj5BfLoa` READY; stabilní development alias na něj ukazuje. Metadata developmentu jsou jednotně `1.7.86`, SW cache `v1.7.86`, build `v1.7.86-shift-calendars1`; release evidence má artefakt ID `10815820131` a rollback míří na READY `dpl_8QFJuAL4bwpfsVF6UkrHrxuPzX3c`. `main` je `de443b771bb7e7dd5fefa498883fdd220a78f07d`; produkční Vercel zůstává `dpl_3Sn4PbVPMSAF2yrUTXKphoDEZ6tj` a produkční Supabase `bkqamcbkiwumsvelahxr`. Úplný fyzický checklist produkce 1.7.83 je potvrzený; fyzická přejímka aktuálního developmentu 1.7.86 zůstává otevřená. Vývojové práce pokračují pouze na `development`; další funkční změny nejprve do TEST `cgshssdjgzzuprlwnabl`.
...
**Stav 1.7.86:** implementováno a vydáno do TEST. Kalendáře mají vlastní administraci, A/B/C/D podporuje jeden i více Google Calendar embed odkazů, Dashboard používá směnu aktivního účtu, stávající kalendář je před prvním uložením fallbackem D a privátní ICS URL je z bezpečnostních důvodů odmítnuta. CI/Chromium/TEST HTTP jsou PASS; fyzický iPhone test správného mapování a více kalendářů ještě čeká. Procenta P2.2/P0.3 se zatím nemění, protože jejich otevřené checkboxy jsou širší než tento funkční balík.
...
7. **První nový TEST balík – základ implementován v 1.7.84 a nesen aktuálním 1.7.86, fyzická přejímka čeká:** local-first Dashboard/start, oddělení stavu účtů, account-scoped sync vzhledu s revizí/CAS a syntetický owner invariant jsou automaticky regresně ověřené. Před uzavřením potvrdit iPhone A→B / offline / dva přístroje / owner checklist; Chromium za něj nepovažovat.
8. **Druhý TEST balík – implementován jako 1.7.86, čeká fyzická přejímka:** směnové kalendáře a jejich bezpečné mapování na rozpis/účty mimo rozpis jsou na preview `dpl_HBdcUbvP34mxj6mH1hJmdj5BfLoa`, Actions #280 PASS. Před uzavřením potvrdit na iPhonu D fallback, A/B/C/D mapování, více kalendářů a odmítnutí privátního ICS.
