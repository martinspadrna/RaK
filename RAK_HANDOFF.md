...
- Současný ověřený development runtime je `1.7.115`. Dokumentační nebo čistě testovací následník bez změny runtime číslo verze nezvyšuje.
...
- Poslední ověřený **funkční** development release je `1.7.115` na SHA `273b07e2e253155f80463caeddb6cec9b83ed3c7`. [Actions #393](https://github.com/martinspadrna/RaK/actions/runs/36229821686) je GREEN na první pokus a stable TEST alias ukazuje na `dpl_3N1z3LKfXNwsrnpY2fywt2bgLsrC`. Vlastník fyzicky potvrdil 1.7.114 svislou nabídku `Neplánovaná dovolená` / `Odebrat` jako OK a pokračoval k poli Důvod. 1.7.115 proto řeší stále jediný úkol **Neplánovaná změna (generátor)**: pole Důvod je pevný select přesně s pěti možnostmi `Dovolená`, `Náhradní volno`, `Paragraf`, `Lékař`, `Odešel na kalírnu`; žádné další důvody ani volné psaní. První čtyři jsou absence s částečným přepočtem, Kalírna je skutečný `dayMods: kalirnaOut` bez falešné absence. Čeká fyzická iPhone přejímka tohoto výběru.
...
- Stabilní development alias nyní ukazuje na `dpl_3N1z3LKfXNwsrnpY2fywt2bgLsrC`, READY / SHA `273b07e2e253155f80463caeddb6cec9b83ed3c7`. [Actions #393](https://github.com/martinspadrna/RaK/actions/runs/36229821686) je SUCCESS na první pokus. Dva canonical buildy, npm check, nové reason/RPC gate testy, rollback/ZIP/CRC, mobilní Chromium/offline/recovery, performance budgety/parita, quality thresholds, immutable HTTP i stable-alias HTTP jsou PASS. Performance parity: startupReady P50 `414→406 ms`, P95 `467→455 ms`; FCP P50 `304→320 ms`, P95 `312→348 ms`. PWA budgety: cold mobile P50/P95 `1536/1548 ms`, offline reload `1222/1248 ms`, online recovery `579/588 ms`. Release evidence `rak-release-evidence-273b07e2e253155f80463caeddb6cec9b83ed3c7`, ID `10902501925`, SHA-256 `a898584f7299a6602726b99d462fb759afa5f0e71ed876d43740b54c01f9563d`; CI proof ID `10901682492`, SHA-256 `3949432d8e83630cc2aa78e06331379e46d0897d4934fc90adaa90938442ded3`; historický isolated-build alias ID `10901917204`, SHA-256 `75f5ce6e0ace35622ebf71e78c4cd40c471d965ecf75a9799937c60c026e92ca`. Rollback cíl je předchozí READY `dpl_3AtPYadzSWdrb3aBYPCTnR7ZyPAQ`. TEST Supabase má aplikovanou migraci `20260926081926_rak_unplanned_reason_catalog_17115`; rollback probe ověřil Kalírna CAS, idempotentní retry a odmítnutí nepovoleného důvodu `N`, bez zanechání změny. Nové RPC není anon-executable. Produkce ani `main` se nezměnily.
...
### AKTUÁLNÍ JEDINÝ ÚKOL – 26. 9. 2026, 1.7.115 výběr Důvodu

Od tohoto bodu stále platí přání vlastníka **striktně po jednom úkolu**. Další backlog se nesmí rozpracovat, dokud nebude přijatý tok Neplánované změny.

- 1.7.114 svislou akční nabídku vlastník fyzicky potvrdil jako **OK** větou „Ok, teď v těch důvodech…“; tento layout detail je uzavřený.
- Ověřený runtime je **1.7.115** na SHA `273b07e2e253155f80463caeddb6cec9b83ed3c7`, build `v1.7.115-unplanned-reasons1`.
- [Actions #393](https://github.com/martinspadrna/RaK/actions/runs/36229821686) je **SUCCESS na první pokus**. Stable TEST alias ukazuje na `dpl_3N1z3LKfXNwsrnpY2fywt2bgLsrC`; rollback je `dpl_3AtPYadzSWdrb3aBYPCTnR7ZyPAQ`.
- Pole **Důvod** už není textový input/datalist. Je to pevný select přesně s pěti položkami schválenými vlastníkem: **Dovolená**, **Náhradní volno**, **Paragraf**, **Lékař**, **Odešel na kalírnu**. Žádné další položky se tam nesmí vrátit bez nového zadání.
- Dovolená/NV/Paragraf/Lékař používají stávající částečný generátor absence a serverovou v1 ochranu. **Odešel na kalírnu** se neukládá jako absence: klient vytvoří `dayMods` záznam `kalirnaOut` na skutečně přiřazené buňce a řádky rozpisu zůstanou beze změny.
- TEST migrace `20260926081926_rak_unplanned_reason_catalog_17115` přidává `rak_admin_apply_unplanned_change_v2`: admin-only, CAS-bound, idempotentní; absence allowlist jen `D/NV/§/LEK`, daymod allowlist jen `kalirnaOut`, žádný EXECUTE pro anon. Rollback probe po migraci PASS.
- Performance parity i celý release řetězec prošly bez rerunu. Release evidence ID `10902501925`, SHA-256 `a898584f7299a6602726b99d462fb759afa5f0e71ed876d43740b54c01f9563d`.
- **Teď jen fyzická iPhone kontrola nabídky Důvod.** Další krok až podle reakce vlastníka; nepřidávat jiný úkol.
