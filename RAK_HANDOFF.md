...
- Současný ověřený development runtime je `1.7.114`. Dokumentační nebo čistě testovací následník bez změny runtime číslo verze nezvyšuje.
...
- Poslední ověřený **funkční** development release je `1.7.114` na SHA `85a98d583ed464b645342f53422c1ba7e874596e`. [Actions #392](https://github.com/martinspadrna/RaK/actions/runs/36227808743) je GREEN na druhý pokus stejného SHA a stable TEST alias ukazuje na `dpl_3AtPYadzSWdrb3aBYPCTnR7ZyPAQ`. Jediný aktivní úkol zůstává **Neplánovaná změna (generátor)**. Fyzický screenshot 1.7.113 ukázal, že vnější box už je svislý, ale vnitřní `.adminRotationQuickRemoveActions` se stále řadí vodorovně. 1.7.114 proto při každém otevření vynucuje inline `!important` přímo na kontejneru akcí i obou tlačítkách (`display:grid`, `1fr`, `width:100%`). Čeká pouze fyzická iPhone přejímka tohoto jediného detailu.
...
- Stabilní development alias nyní ukazuje na `dpl_3AtPYadzSWdrb3aBYPCTnR7ZyPAQ`, READY / SHA `85a98d583ed464b645342f53422c1ba7e874596e`. [Actions #392](https://github.com/martinspadrna/RaK/actions/runs/36227808743) je SUCCESS na druhý pokus stejného SHA; první verify pokus zastavila jen performance parita kvůli neobvykle rychlému baseline FCP (`288 ms` baseline vs `328 ms` current, limit `317 ms`). Bez změny kódu nebo limitů rerun prošel. Finální parity PASS: startupReady P50 `423→405 ms`, P95 `456→450 ms`; FCP P50 `324→308 ms`, P95 `332→340 ms`. PWA budgety PASS: cold mobile P50/P95 `1596/1997 ms`, offline reload `1249/1290 ms`, online recovery `575/579 ms`. Release evidence `rak-release-evidence-85a98d583ed464b645342f53422c1ba7e874596e`, ID `10901084671`, SHA-256 `b14debe18abb5198c9d0417181bd8965ca12944d7cf7607158be8af35da89f41`; CI proof ID `10901303700`, SHA-256 `2ac2795281f03129e822ed632a3f1f9a61e659fb426ef7a812f9df8d05d0b597`; historický isolated-build alias ID `10902045080`, SHA-256 `85b9c89b5813d9a10e58e9c3c73e3dfae3ea0e60fe5e37becd2eb3586f417d23`. Rollback cíl je předchozí READY `dpl_9ztNpFUsYJdvApL7hd3D5865vsTk`. Produkce ani `main` se nezměnily.
...
### AKTUÁLNÍ JEDINÝ ÚKOL – 26. 9. 2026, 1.7.114 Neplánovaná dovolená / Odebrat
...
- Ověřený runtime je **1.7.114** na SHA `85a98d583ed464b645342f53422c1ba7e874596e`, build `v1.7.114-unplanned-menu-inline1`.
- [Actions #392](https://github.com/martinspadrna/RaK/actions/runs/36227808743) je SUCCESS na druhý verify pokus stejného SHA bez změny kódu nebo limitů. Stable TEST alias ukazuje na `dpl_3AtPYadzSWdrb3aBYPCTnR7ZyPAQ`; rollback je `dpl_9ztNpFUsYJdvApL7hd3D5865vsTk`.
- Fyzický screenshot 1.7.113 ukázal: vnější box je už svislý (jméno nahoře), ale vnitřní blok tlačítek zůstává vodorovný. Kořenová příčina je staré `.adminRotationQuickRemoveActions{display:flex!important}`.
- 1.7.114 při každém otevření nastavuje přímo inline `!important` na `.adminRotationQuickRemoveActions`: `display:grid`, `grid-template-columns:1fr`, `width:100%`, `gap:8px`; obě tlačítka dostanou inline `display:block`, `width:100%`, `min-width:0` a zalamování textu.
- Tohle je záměrně silnější než všechny staré stylesheety; velký popup a serverová logika se nemění.
- Release evidence ID `10901084671`, SHA-256 `b14debe18abb5198c9d0417181bd8965ca12944d7cf7607158be8af35da89f41`.
