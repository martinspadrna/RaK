...
- Současný ověřený development runtime je `1.7.87`. Dokumentační nebo čistě testovací následník bez změny runtime číslo verze nezvyšuje.
...
- Poslední ověřený **funkční** development release: `2443abfcf6e13e01e0f26aef1e5f3e5d15b85c2d` (`1.7.87`). Živý HEAD se vždy zjišťuje online; případný následný dokumentační commit nesmí být zaměněn za nový runtime release.
...
- Stabilní development alias nyní ukazuje na `dpl_5zqBT2g5jDAZmJ9fNyxgdygznPn6`, READY / SHA `2443abfcf6e13e01e0f26aef1e5f3e5d15b85c2d`. Přesný SHA prošel [Actions #283](https://github.com/martinspadrna/RaK/actions/runs/36025325916) SUCCESS; release job přesunul alias až po celé fail-closed bráně. Ověřený rollback cíl je READY `dpl_HBdcUbvP34mxj6mH1hJmdj5BfLoa` / předchozí development 1.7.86.
- Release 1.7.87 prošel dvěma kanonickými buildy, celým `npm run check`, zděděnými i novými gate testy, rollback/ZIP důkazy, reálným Chromium online→offline→online testem, třemi PWA benchmarky a živou anonymní TEST HTTP kontrolou. Release evidence `rak-release-evidence-2443abfcf6e13e01e0f26aef1e5f3e5d15b85c2d` má artefakt ID `10818973416`. Předchozí Actions #282 selhalo pouze na staré testovací VM fixture; 1.7.87 public-ICS testy v něm už byly PASS a test-only commit `2443abfc...` fixture opravil bez dalšího zvýšení verze.
- Development metadata: viditelná `1.7.87`, technická `1.7.87`, modulová cache `1.7.87`, `package.json` `1.7.87`, SW cache `v1.7.87`, build `v1.7.87-public-ics1`. TEST Supabase zůstává `cgshssdjgzzuprlwnabl`; release 1.7.87 nepřidal databázovou migraci a používá existující autentizovanou cestu `rak_admin_save_machine_settings_v2` pro konfiguraci směnových kalendářů. Dřívější account-scoped appearance CAS migrace `rak_account_ui_preferences_cas_17084` zůstává pouze v TEST.
...
**RaK 1.7.87 – fyzická přejímka aktuálního developmentu ještě čeká.** Automatické Chromium/CI důkazy se za iPhone nevydávají. Release 1.7.85 opravil doručování PWA aktualizací a vlastník následně potvrdil, že se development skutečně aktualizoval na 1.7.85; tím je doložena update cesta, nikoli celý fyzický checklist local-first/account syncu.

Na stabilním development aliasu 1.7.87 je potřeba potvrdit zejména: (1) online návrat do instalované PWA zobrazí „kam jdu“ bez čekání na Supabase; (2) úplné zavření a start v režimu Letadlo zobrazí stejný lokální rozpis; (3) účet A → odhlásit → účet B ani krátce neukáže jméno nebo vzhled A; (4) stejný účet na dvou zařízeních převezme serverový vzhled a offline změna se po reconnectu deterministicky vyřeší bez globálního konfliktu; (5) přesun owner účtu mimo rozpis a zpět nezmění Auth/owner oprávnění; (6) Administrace → Informace pro zaměstnance → Kalendáře správně odděluje A/B/C/D, existující D kalendář zůstává jako fallback před prvním uložením, více kalendářů na jedné směně lze přepnout, veřejný `public/basic.ics` se uloží a zobrazí po automatickém převodu na embed a soukromý private ICS odkaz je odmítnut. Do tohoto potvrzení se žádný iPhone-dependent checkbox nezavírá.
...
**Aktuální online stav k 24. 9. 2026:** produkční předání 1.7.83 včetně schválené bezpečnostní fáze B je dokončeno a produkce zůstává beze změny. Nejnovější TEST runtime je 1.7.87 na funkčním SHA `2443abfcf6e13e01e0f26aef1e5f3e5d15b85c2d`, [Actions #283](https://github.com/martinspadrna/RaK/actions/runs/36025325916) SUCCESS a Vercel `dpl_5zqBT2g5jDAZmJ9fNyxgdygznPn6` READY; stabilní development alias na něj ukazuje. Metadata developmentu jsou jednotně `1.7.87`, SW cache `v1.7.87`, build `v1.7.87-public-ics1`; release evidence má artefakt ID `10818973416` a rollback míří na READY `dpl_HBdcUbvP34mxj6mH1hJmdj5BfLoa`. `main` je `de443b771bb7e7dd5fefa498883fdd220a78f07d`; produkční Vercel zůstává `dpl_3Sn4PbVPMSAF2yrUTXKphoDEZ6tj` a produkční Supabase `bkqamcbkiwumsvelahxr`. Úplný fyzický checklist produkce 1.7.83 je potvrzený; fyzická přejímka aktuálního developmentu 1.7.87 zůstává otevřená. Vývojové práce pokračují pouze na `development`; další funkční změny nejprve do TEST `cgshssdjgzzuprlwnabl`.
...
**Stav 1.7.87:** implementováno a vydáno do TEST. Kalendáře mají vlastní administraci, A/B/C/D podporuje jeden i více Google Calendar odkazů, Dashboard používá směnu aktivního účtu a stávající kalendář je před prvním uložením fallbackem D. Povolen je standardní Google embed i veřejný `.../public/basic.ics`; veřejný ICS se před uložením automaticky převede na embed URL, zatímco private ICS s neveřejným tokenem zůstává odmítnutý. CI/Chromium/TEST HTTP jsou PASS; fyzický iPhone test správného mapování, více kalendářů a public ICS ještě čeká. Procenta P2.2/P0.3 se zatím nemění, protože jejich otevřené checkboxy jsou širší než tento funkční balík.
...
**Stav v aktuálním 1.7.86 (základ implementován v 1.7.84):** první tři provozní rizika tohoto bloku mají nový systémový základ: při přepnutí účtu se nejdřív odstraní stará runtime identita bez plošného mazání storage, vzhled je account-scoped a synchronizovaný přes serverovou revizi/CAS a syntetická regrese dokazuje, že změna směny/rosteru nemění owner/Auth identitu. Fyzický A→B test, skutečný dvouzařízení sync/offline konflikt a obecné úpravy UI Pracovníci/Správci zůstávají otevřené.
...
7. **První nový TEST balík – základ implementován v 1.7.84 a nesen aktuálním 1.7.87, fyzická přejímka čeká:** local-first Dashboard/start, oddělení stavu účtů, account-scoped sync vzhledu s revizí/CAS a syntetický owner invariant jsou automaticky regresně ověřené. Před uzavřením potvrdit iPhone A→B / offline / dva přístroje / owner checklist; Chromium za něj nepovažovat.
8. **Druhý TEST balík – implementován v 1.7.86 a opraven public-ICS releasem 1.7.87, čeká fyzická přejímka:** směnové kalendáře a jejich bezpečné mapování na rozpis/účty mimo rozpis jsou na preview `dpl_5zqBT2g5jDAZmJ9fNyxgdygznPn6`, Actions #283 PASS. Před uzavřením potvrdit na iPhonu D fallback, A/B/C/D mapování, více kalendářů, uložení veřejného `public/basic.ics` a odmítnutí privátního ICS.
