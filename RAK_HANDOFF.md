...
- Současný ověřený development runtime je `1.7.90`. Dokumentační nebo čistě testovací následník bez změny runtime číslo verze nezvyšuje.
...
- Poslední ověřený **funkční** development release: `404637600413067219a7242e1a26ecdc6fc68167` (`1.7.90`). Živý HEAD se vždy zjišťuje online; případný následný dokumentační commit nesmí být zaměněn za nový runtime release.
...
- Stabilní development alias nyní ukazuje na `dpl_CVHENG3ePY24ZdbS8Z2remGgbp1z`, READY / SHA `404637600413067219a7242e1a26ecdc6fc68167`. Přesný SHA prošel [Actions #291](https://github.com/martinspadrna/RaK/actions/runs/36033861154) SUCCESS; release job přesunul alias až po celé fail-closed bráně. Ověřený rollback cíl je READY `dpl_F8L2xx9ZzYF3hFtvfVyniouWard3` / předchozí development 1.7.89.
- Release 1.7.90 prošel dvěma kanonickými buildy, celým `npm run check`, zděděnými i novými gate testy, bezpečnostní kontrolou nového endpointu, rollback/ZIP důkazy, reálným Chromium online→offline→online testem, třemi PWA benchmarky a živou anonymní TEST HTTP kontrolou. Release evidence `rak-release-evidence-404637600413067219a7242e1a26ecdc6fc68167` má artefakt ID `10822664323`.
- Development metadata: viditelná `1.7.90`, technická `1.7.90`, modulová cache `1.7.90`, `package.json` `1.7.90`, SW cache `v1.7.90`, build `v1.7.90-native-public-calendar1`. Google iframe byl z Dashboard kalendáře odstraněn; veřejný ICS se načítá přes úzce omezený same-origin endpoint `/api/public-calendar` a vykresluje nativně v RaK. Endpoint přijímá pouze Google calendar ID, nepřijímá libovolnou URL ani private ICS a má limit velikosti/timeout. Žádná nová Supabase migrace nebyla potřeba. Živý kontrolní veřejný Google kalendář i směna D přes endpoint vracejí HTTP 200/VCALENDAR; aktuální kalendáře A/B/C vracejí upstream nedostupnost, takže jejich Google public-ICS zpřístupnění je konfigurační blokátor mimo RaK.
...
**RaK 1.7.90 – fyzická přejímka aktuálního developmentu ještě čeká.** Automatické Chromium/CI důkazy se za iPhone nevydávají. Release 1.7.85 opravil doručování PWA aktualizací a vlastník následně potvrdil, že se development skutečně aktualizoval na 1.7.85; tím je doložena update cesta, nikoli celý fyzický checklist local-first/account syncu.

Na stabilním development aliasu 1.7.90 je potřeba potvrdit zejména: (1) online návrat do instalované PWA zobrazí „kam jdu“ bez čekání na Supabase; (2) úplné zavření a start v režimu Letadlo zobrazí stejný lokální rozpis; (3) účet A → odhlásit → účet B ani krátce neukáže jméno nebo vzhled A; (4) stejný účet na dvou zařízeních převezme serverový vzhled a offline změna se po reconnectu deterministicky vyřeší bez globálního konfliktu; (5) přesun owner účtu mimo rozpis a zpět nezmění Auth/owner oprávnění; (6) Kalendáře: screenshoty z 24. 9. fyzicky potvrzují, že účet nastavený na A už otevírá „Kalendář · směna A“, modal má správnou výšku a × je vycentrované. Stejný test ale odhalil blokaci Google iframe kvůli cookie/přihlášení; 1.7.90 proto iframe odstranila a používá nativní měsíční kalendář z public ICS. Před uzavřením je nutný iPhone retest nativního UI. Směna D má živě ověřený public ICS; A/B/C aktuálně Google přes public ICS nevydává, takže před jejich retestem musí být v Google skutečně zpřístupněné jako veřejné kalendáře nebo se musí zvolit jiný bezpečný způsob distribuce. Do tohoto potvrzení se žádný širší iPhone-dependent checkbox nezavírá.
...
**Aktuální online stav k 24. 9. 2026:** produkční předání 1.7.83 včetně schválené bezpečnostní fáze B je dokončeno a produkce zůstává beze změny. Nejnovější TEST runtime je 1.7.90 na funkčním SHA `404637600413067219a7242e1a26ecdc6fc68167`, [Actions #291](https://github.com/martinspadrna/RaK/actions/runs/36033861154) SUCCESS a Vercel `dpl_CVHENG3ePY24ZdbS8Z2remGgbp1z` READY; stabilní development alias na něj ukazuje. Metadata developmentu jsou jednotně `1.7.90`, SW cache `v1.7.90`, build `v1.7.90-native-public-calendar1`; release evidence má artefakt ID `10822664323` a rollback míří na READY `dpl_F8L2xx9ZzYF3hFtvfVyniouWard3`. TEST Supabase zůstává `cgshssdjgzzuprlwnabl` s poslední migrací login-shift v3 z 1.7.89; 1.7.90 databázi nemění. `main` je `de443b771bb7e7dd5fefa498883fdd220a78f07d`; produkční Vercel zůstává `dpl_3Sn4PbVPMSAF2yrUTXKphoDEZ6tj` a produkční Supabase `bkqamcbkiwumsvelahxr`. Fyzický screenshot potvrdil mapování směny A, výšku modalu a centrování × z 1.7.89; nativní kalendář 1.7.90 ještě čeká na iPhone test a A/B/C mají před tím konfigurační blokátor public ICS na straně Google. Vývojové práce pokračují pouze na `development`; další funkční změny nejprve do TEST `cgshssdjgzzuprlwnabl`.
