...
- Poslední ověřený **funkční** development release před touto dokumentační aktualizací: `29e07d975722b08a185589dfd37cce640436c558`. Živý HEAD se vždy zjišťuje online; následný dokumentační commit nesmí být zaměněn za nový runtime release.
...
- Stabilní development alias nyní ukazuje na `dpl_9tk5pjCu698W3rRqGKdVYXENwCeS`, READY / SHA `29e07d975722b08a185589dfd37cce640436c558`. Přesný SHA prošel [Actions #274](https://github.com/martinspadrna/RaK/actions/runs/36007757382) SUCCESS; release job alias přesunul až po celé fail-closed bráně. Ověřeným rollback cílem pro tento release je READY `dpl_FrVbTCjNnuuHevvtDFWVv8XTYYZg`.
- Poslední funkční/databázová validace: [Actions #274](https://github.com/martinspadrna/RaK/actions/runs/36007757382), SUCCESS na SHA `29e07d975722b08a185589dfd37cce640436c558`. Prošla dvěma kanonickými buildy, celým `npm run check`, rollback/ZIP důkazy, reálným Chromium online→offline→online testem, třemi PWA benchmarky a živou anonymní TEST HTTP kontrolou. Jediný preview `dpl_9tk5pjCu698W3rRqGKdVYXENwCeS` je READY na témže SHA; stabilní development alias je veřejně dostupný a release evidence `rak-release-evidence-29e07d975722b08a185589dfd37cce640436c558` má artefakt ID `10810743243`.
- Development metadata po tomto balíku: viditelná `1.7.84`, technická `1.7.0`, cache `v1.7.84`, build `v1.7.84-local-first-account-sync1`. TEST Supabase `cgshssdjgzzuprlwnabl` má aplikovanou migraci `rak_account_ui_preferences_cas_17084`; rollback-only CAS sonda i volání omezených RPC pod rolí `anon` prošly. Přímé klientské SELECT/INSERT/UPDATE nad novou tabulkou zůstávají odebrané.
...
**RaK 1.7.84 – fyzická přejímka ještě čeká.** Automatické Chromium/CI důkazy se za iPhone nevydávají. Na stabilním development aliasu je potřeba potvrdit zejména: (1) online návrat do instalované PWA zobrazí „kam jdu“ bez čekání na Supabase; (2) úplné zavření a start v režimu Letadlo zobrazí stejný lokální rozpis; (3) účet A → odhlásit → účet B ani krátce neukáže jméno nebo vzhled A; (4) stejný účet na dvou zařízeních převezme serverový vzhled a offline změna se po reconnectu deterministicky vyřeší bez globálního konfliktu; (5) přesun owner účtu mimo rozpis a zpět nezmění Auth/owner oprávnění. Do tohoto potvrzení se žádný iPhone-dependent checkbox nezavírá.
...
**Aktuální online stav k 24. 9. 2026:** produkční předání 1.7.83 včetně schválené bezpečnostní fáze B je dokončeno. Nejnovější TEST runtime je 1.7.84 na funkčním SHA `29e07d975722b08a185589dfd37cce640436c558`, Actions #274 SUCCESS a Vercel `dpl_9tk5pjCu698W3rRqGKdVYXENwCeS` READY; fyzická přejímka 1.7.84 ještě čeká. `main` je `de443b771bb7e7dd5fefa498883fdd220a78f07d`; povinná validace [Actions #261](https://github.com/martinspadrna/RaK/actions/runs/35957793587) je SUCCESS a ruční [produkční release #1](https://github.com/martinspadrna/RaK/actions/runs/35958452867) je SUCCESS. Vercel deployment `dpl_3Sn4PbVPMSAF2yrUTXKphoDEZ6tj` je READY na přesném main SHA a produkční alias na něj ukazuje; metadata jsou `1.7.83` / `1.7.0` / cache `v1.7.83` / build `v1.7.83-about-release1`. Produkční Supabase zůstává projekt `bkqamcbkiwumsvelahxr`; fáze A i atomická fáze B `20260924104723_rak_production_phase_b_17083` jsou aplikované a `rak-admin-users` je ACTIVE ve verzi 8 s `verify_jwt=true`. Úplný fyzický checklist 1.7.83 je potvrzený. Následný vlastníkem schválený úklid byl po zeleném Actions #269 aplikován jako `20260924105811_rak_delete_retired_gomoku_history_17083`: `gomoku_wins` má nula řádků, `game_accounts` zůstává 11 a tabulka je nadále bez klientského čtení i zápisu. Vývojové práce pokračují pouze na `development`; další funkční změny nejprve do TEST `cgshssdjgzzuprlwnabl`.
...
7. **První nový TEST balík – implementován jako 1.7.84, čeká fyzická přejímka:** local-first Dashboard/start, oddělení stavu účtů, account-scoped sync vzhledu s revizí/CAS a syntetický owner invariant jsou na preview `dpl_9tk5pjCu698W3rRqGKdVYXENwCeS` a CI #274 PASS. Před uzavřením balíku potvrdit uvedený iPhone A→B / offline / dva přístroje / owner checklist; Chromium za něj nepovažovat.
