...
assert(['v1.7.66-softgrid-draftguard1','v1.7.67-equalgrid-reload1','v1.7.68-async-draft-guard1','v1.7.69-local-drafts1','v1.7.70-canonical-source1']
...
try{
 ...
 assert(Math.abs(data.table-324)<=2,fail('MO table is stretched'));
 assert(data.cellWidths[0]>=82&&data.cellWidths[0]<=86,fail('date col wrong'));
 assert(data.cellWidths.slice(1).every(n=>n>=46&&n<=50),fail('MO name cells not compact'));
 assert(data.nameWidths.every(n=>n>=45&&n<=47),fail('MO name input wrong'));
