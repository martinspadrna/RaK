...
test('P2.1 parity uses immutable 1.7.69, its historical two-pass build and common browser metrics',()=>{
 ...
 const currentMarker='current-'+config.current.version;
 for(const marker of ["git',['-C',WORKSPACE,'worktree','add','--detach'","npm',['run','vercel-build']","window.__rakBootV2StartupReady","first-contentful-paint","dashboardVisible","baseline-1.7.69",currentMarker])assert(script.includes(marker),'missing '+marker);
