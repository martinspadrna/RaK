...
test('build evidence proves two stable passes without source changes when present',()=>{
  ...
  assert.equal(value.release,'1.7.74');
  assert.equal(value.buildId,'v1.7.74-offline-cache1');
  assert.match(value.stableDigest,/^[a-f0-9]{64}$/);
  assert.deepEqual(value.variableOutputs,['rak-complete-backup-source.zip']);
  assert(read('.rak-dist/index.html').includes("var build='v1.7.74-offline-cache1';"));
  assert(read('.rak-dist/sw.js').includes("const CACHE_VERSION = 'v1.7.74';"));
  assert(read('.rak-dist/supabase-config.js').includes('window.RAK_RELEASE_VERSION = "1.7.74";'));
