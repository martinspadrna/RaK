...
if(canonical){
  assert(sw.includes("const DEVELOPMENT_TEST_DISPLAY_VERSION = '1.7.73';"), 'canonical SW display version must be 1.7.73');
  assert(config.includes('window.RAK_RELEASE_VERSION = "1.7.73";'), 'canonical display release version must be 1.7.73');
  assert(config.includes('window.RAK_TEST_DISPLAY_VERSION = "1.7.73";'), 'canonical test display version must be 1.7.73');
  assert(config.includes('window.RAK_PWA_BUILD = "v1.7.73-offline-persistence1";'), 'canonical PWA build marker missing');
}else{
