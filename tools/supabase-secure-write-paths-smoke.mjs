...
assert(runtimeBuildSource.includes(`window.RAK_RELEASE_VERSION = "${devDisplayVersion}";`) || runtimeBuildSource.includes(`window.RAK_RELEASE_VERSION=TEST_BUILD;`), 'development display release version must match effective runtime build');
assert(runtimeBuildSource.includes(`window.RAK_TEST_DISPLAY_VERSION = "${devDisplayVersion}";`) || runtimeBuildSource.includes(`window.RAK_TEST_DISPLAY_VERSION=TEST_BUILD;`), 'development test display version must match effective runtime build');
assert(runtimeBuildSource.includes(`window.RAK_PWA_BUILD = "v${devDisplayVersion}";`) || runtimeBuildSource.includes(`window.RAK_PWA_BUILD='v'+TEST_BUILD;`), 'development PWA build marker must match effective runtime build');
assert(loginLife.includes(`const TEST_BUILD='${devDisplayVersion}'`), 'runtime test-build override must match SW display version');
