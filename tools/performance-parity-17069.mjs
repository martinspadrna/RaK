...
try{
  ...
  assert(currentMetadata.includes("displayVersion: '1.7.106'"),'[perf-parity] current canonical release metadata is not 1.7.106');
  assert.equal(currentPackage.version,'1.7.106','[perf-parity] current canonical package is not 1.7.106');
  const baseline=[],current=[];
  for(let round=1;round<=CONFIG.rounds;round++){baseline.push(await measureRoot(baselineRoot,'baseline-1.7.69',round));current.push(await measureRoot(ROOT,'current-1.7.106',round));}
