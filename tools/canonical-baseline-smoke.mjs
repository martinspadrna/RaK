...
test('canonical release metadata is one explicit 1.7.70 preview identity',()=>{
  ...
  assert(read('sw.js').includes("const DEVELOPMENT_TEST_DISPLAY_VERSION = '1.7.70';"));
  assert(read('sw.js').includes("const DEVELOPMENT_BUILD_ID = 'v1.7.70-canonical-source1';"));
  assert(read('app.js').includes('const RAK_DEV_UPDATE_BUILD = "v1.7.70-canonical-source1";'));
  assert(read('app.js').includes('window.RAK_RELEASE_VERSION = "1.7.70";'));
  assert(read('supabase-config.js').includes('window.RAK_RELEASE_VERSION = "1.7.70";'));
  assert(read('supabase-config.js').includes('window.RAK_TEST_DISPLAY_VERSION = "1.7.70";'));
  assert(read('supabase-config.js').includes('window.RAK_PWA_BUILD = "v1.7.70-canonical-source1";'));
});
