...
DO $setup$ DECLARE v_owner text; v_spoof text; BEGIN
 SELECT jsonb_build_object('sub',p.user_id,'session_id',s.id,'role','authenticated')::text INTO v_owner FROM public.rak_admin_profiles p JOIN auth.sessions s ON s.user_id=p.user_id WHERE p.role='owner' AND p.enabled ORDER BY s.created_at DESC LIMIT 1;
 ...
 PERFORM set_config('rak.v9_owner',v_owner,true); PERFORM set_config('rak.v9_spoof',v_spoof,true);
 ...
 SELECT 'RAK_RECURSIVE_TEST_'||kind,'synthetic fixture','frezka',jsonb_build_object('wrapper',jsonb_build_object('deep',jsonb_build_object(kind,jsonb_build_array(jsonb_build_object('name','synthetic'))))) FROM unnest(ARRAY['appAccounts','applicationAccounts','workers','loginNumber','accountNumber','roster','employees','staff']) kind;
...
DO $anon$ BEGIN
 ...
 IF NOT public.rak_admin_account_requires_auth('9811') OR public.rak_admin_account_requires_auth('not-an-admin') THEN RAISE EXCEPTION 'Admin auth lookup changed'; END IF;
...
DO $owner$ BEGIN
 IF NOT private.rak_is_admin() OR (SELECT count(*) FROM public.machine_settings WHERE machine_key LIKE 'RAK_RECURSIVE_TEST_%')<>9 OR (SELECT count(*) FROM public.machine_settings WHERE machine_key='WORKER_ROSTER_SETTINGS')<>1 THEN RAISE EXCEPTION 'Verified owner visibility failed'; END IF;
END $owner$;
RESET ROLE;
UPDATE public.rak_admin_profiles SET enabled=false WHERE account_id='9811';
SET LOCAL ROLE anon;
DO $disabled$ BEGIN IF public.rak_admin_account_requires_auth('9811') THEN RAISE EXCEPTION 'Disabled owner still passes public Auth lookup'; END IF; END $disabled$;
ROLLBACK;
SELECT 'PASS: eight nested personal-data fixtures denied, ordinary settings/owner/login/rotation retained, disabled account honored, no persistent writes' AS result;
