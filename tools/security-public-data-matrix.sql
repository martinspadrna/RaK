...
BEGIN
 ...
 IF v_rpc IS DISTINCT FROM ARRAY['rak_admin_account_requires_auth','rak_admin_auth_capabilities','rak_app_keepalive','rak_lookup_account_for_login_v1','rak_submit_bug_report_v2']::text[] THEN RAISE EXCEPTION 'Public RPC surface changed: %',v_rpc; END IF;
...
END $owner$;
ROLLBACK;
SELECT 'PASS: 3 public tables, 5 RPCs, zero public storage, live notices only, recursive roster hidden, owner archive and legacy rotation preserved; rollback' AS result;
