...
function fixture() {
  ...
  return {
    ...
    'RAK_HANDOFF.md':ids.map(id=>`| ${id} | item |`).join('\n'),
...
test('missing roadmap item fails closed',()=>{const f=fixture();f['RAK_HANDOFF.md']=f['RAK_HANDOFF.md'].replace(/^\| P2\.4 \|.*$/m,'');assert.throws(()=>assertReleaseSnapshot(f),/thirteen unique/);});
