...
- Současný ověřený development runtime je `1.7.91`. Dokumentační nebo čistě testovací následník bez změny runtime číslo verze nezvyšuje.
...
- Poslední ověřený **funkční** development release: `01575bc779924e74aa141c46394d08dc88f3df37` (`1.7.91`). Živý HEAD se vždy zjišťuje online; případný následný dokumentační commit nesmí být zaměněn za nový runtime release.
...
- Stabilní development alias nyní ukazuje na `dpl_8nw6WHHdocmYY78weqf2iD4oSviV`, READY / SHA `01575bc779924e74aa141c46394d08dc88f3df37`. Přesný SHA prošel [Actions #293](https://github.com/martinspadrna/RaK/actions/runs/36035802452) SUCCESS; release job přesunul alias až po celé fail-closed bráně. Ověřený rollback cíl je READY `dpl_CVHENG3ePY24ZdbS8Z2remGgbp1z` / předchozí development 1.7.90.
- Release 1.7.91 prošel dvěma kanonickými buildy, celým `npm run check`, zděděnými i novými gate testy, rollback/ZIP důkazy, reálným Chromium online→offline→online testem, třemi PWA benchmarky a živou anonymní TEST HTTP kontrolou. Release evidence `rak-release-evidence-01575bc779924e74aa141c46394d08dc88f3df37` má artefakt ID `10825261349`, SHA-256 `8077b1827212076c1b3517e9b69288ab120eacebe00ecb195f99973eacb1720e`.
- Development metadata: viditelná `1.7.91`, technická `1.7.91`, modulová cache `1.7.91`, `package.json` `1.7.91`, SW cache `v1.7.91`, build `v1.7.91-calendar-shift-labels1`. Nativní veřejný ICS kalendář z 1.7.90 zůstává zachovaný. Pokud Google v public ICS skryje název směnové události jako `Busy` / `Zaneprázdněn`, RaK zobrazí podle začátku 06:00 → `Ranní`, 18:00 nebo 22:00 → `Noční`; vlastní názvy událostí se nepřepisují, takže směna D si zachovává `Ranní 12h`, `Noční 12h`, `Ranní 8h` apod. Žádná nová Supabase migrace nebyla potřeba. Vlastník po zveřejnění kalendáře fyzicky potvrdil, že kalendář směny A už načítá události; B/C nebyly v této přejímce výslovně ověřeny.
...
**RaK 1.7.91 – úplná fyzická přejímka aktuálního developmentu ještě čeká.** Automatické Chromium/CI důkazy se za iPhone nevydávají. Release 1.7.85 opravil doručování PWA aktualizací a vlastník následně potvrdil, že se development skutečně aktualizoval na 1.7.85; tím je doložena update cesta, nikoli celý fyzický checklist local-first/account syncu.

Na stabilním development aliasu 1.7.91 je potřeba potvrdit zejména: (1) online návrat do instalované PWA zobrazí „kam jdu“ bez čekání na Supabase; (2) úplné zavření a start v režimu Letadlo zobrazí stejný lokální rozpis; (3) účet A → odhlásit → účet B ani krátce neukáže jméno nebo vzhled A; (4) stejný účet na dvou zařízeních převezme serverový vzhled a offline změna se po reconnectu deterministicky vyřeší bez globálního konfliktu; (5) přesun owner účtu mimo rozpis a zpět nezmění Auth/owner oprávnění; (6) Kalendáře: účet nastavený na A už fyzicky otevřel nativní veřejný kalendář a zobrazil události, takže dřívější blokace Google iframe/public ICS pro A je odstraněná. Po 1.7.91 je ještě potřeba na iPhonu potvrdit nové popisky anonymních směn: 06:00 `Ranní`, 18:00/22:00 `Noční`; pojmenované události směny D se nesmějí změnit. B/C nebyly při tomto kroku výslovně ověřeny. Do úplného potvrzení se žádný širší iPhone-dependent checkbox nezavírá.
...
**Aktuální online stav k 24. 9. 2026:** produkční předání 1.7.83 včetně schválené bezpečnostní fáze B je dokončeno a produkce zůstává beze změny. Nejnovější TEST runtime je 1.7.91 na funkčním SHA `01575bc779924e74aa141c46394d08dc88f3df37`, [Actions #293](https://github.com/martinspadrna/RaK/actions/runs/36035802452) SUCCESS a Vercel `dpl_8nw6WHHdocmYY78weqf2iD4oSviV` READY; stabilní development alias na něj ukazuje. Metadata developmentu jsou jednotně `1.7.91`, SW cache `v1.7.91`, build `v1.7.91-calendar-shift-labels1`; release evidence má artefakt ID `10825261349` a rollback míří na READY `dpl_CVHENG3ePY24ZdbS8Z2remGgbp1z`. TEST Supabase zůstává `cgshssdjgzzuprlwnabl` s poslední migrací login-shift v3 z 1.7.89; 1.7.91 databázi nemění. `main` je `de443b771bb7e7dd5fefa498883fdd220a78f07d`; produkční Vercel zůstává `dpl_3Sn4PbVPMSAF2yrUTXKphoDEZ6tj` a produkční Supabase `bkqamcbkiwumsvelahxr`. Vlastník fyzicky potvrdil, že po zveřejnění kalendáře účet směny A nativní kalendář načítá; 1.7.91 navíc nahrazuje anonymní `Busy` podle času začátku za `Ranní`/`Noční`, ale tento nový popisek ještě čeká na fyzický iPhone retest. Vývojové práce pokračují pouze na `development`; další funkční změny nejprve do TEST `cgshssdjgzzuprlwnabl`.
