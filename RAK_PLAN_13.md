...
**Aktuální uživatelská závada:** fyzický iPhone po releasu 1.7.79 potvrdil dvě oddělené věci. Falešný „Konflikt synchronizace“ už po návratu online a restartu nevzniká, takže oddělení offline cache od synchronizační fronty se chová správně. Samotná Rotace však po studeném startu bez internetu stále není načtená. Zároveň po zapnutí internetu běžící aplikace hlásí „Supabase není připojena“ až do restartu. Diagnostika ukázala, že Supabase SDK je parserově načítané z externího `cdn.jsdelivr.net`; pokud tento skript při offline startu selže, klient se po návratu sítě sám znovu nenačte. Současně byla aplikace při offline bootu závislá na pozdějším lazy/idle načtení sync vrstvy, i když durable Rotace už může být lokálně dostupná. Release 1.7.80 proto dělá dvě systémové změny: při cold offline bootu explicitně načte same-origin sync vrstvu a dokončí aplikaci nejnovějšího persisted snapshotu ještě před označením startupu jako ready; a Supabase SDK dostává opakovatelný loader, který po `online` události znovu načte SDK, připraví sync feature a aktivuje synchronizaci bez reloadu stránky. Chromium regrese před offline restartem smaže běžnou HTTP cache, takže externí SDK nemůže být falešně dostupné z browser cache, a po návratu sítě vyžaduje obnovu Supabase bez reloadu. **P2.3 zůstává na 25 %, dokud fyzický iPhone nepotvrdí správný offline rozpis; konfliktová část se zlepšila, ale hlavní offline acceptance ještě neprošla.**
