...
- Současný ověřený development runtime je `1.7.88`. Dokumentační nebo čistě testovací následník bez změny runtime číslo verze nezvyšuje.
...
- Poslední ověřený **funkční** development release: `1c686ec4a5f6a5c6c0faabc06d3af885b7fd8c33` (`1.7.88`). Živý HEAD se vždy zjišťuje online; případný následný dokumentační commit nesmí být zaměněn za nový runtime release.
...
- Stabilní development alias nyní ukazuje na `dpl_J5XfjRw3oShK9NtFQL2ZDA3vhx34`, READY / SHA `1c686ec4a5f6a5c6c0faabc06d3af885b7fd8c33`. Přesný SHA prošel [Actions #285](https://github.com/martinspadrna/RaK/actions/runs/36027186627) SUCCESS; release job přesunul alias až po celé fail-closed bráně. Ověřený rollback cíl je READY `dpl_5zqBT2g5jDAZmJ9fNyxgdygznPn6` / předchozí development 1.7.87.
- Release 1.7.88 prošel dvěma kanonickými buildy, celým `npm run check`, zděděnými i novými gate testy, rollback/ZIP důkazy, reálným Chromium online→offline→online testem, třemi PWA benchmarky a živou anonymní TEST HTTP kontrolou. Release evidence `rak-release-evidence-1c686ec4a5f6a5c6c0faabc06d3af885b7fd8c33` má artefakt ID `10820885390`.
- Development metadata: viditelná `1.7.88`, technická `1.7.88`, modulová cache `1.7.88`, `package.json` `1.7.88`, SW cache `v1.7.88`, build `v1.7.88-calendar-admin-ui1`. TEST Supabase zůstává `cgshssdjgzzuprlwnabl`; release 1.7.88 nepřidal databázovou migraci. Opravuje admin guard stránky Kalendáře, takže Přidat/Odebrat/Načíst/Uložit zůstávají v kalendářovém editoru, a mobilní UI má × a + Přidat vedle sebe s rozestupy mezi směnami.
...
**Aktuální online stav k 24. 9. 2026:** produkční předání 1.7.83 včetně schválené bezpečnostní fáze B je dokončeno a produkce zůstává beze změny. Nejnovější TEST runtime je 1.7.88 na funkčním SHA `1c686ec4a5f6a5c6c0faabc06d3af885b7fd8c33`, [Actions #285](https://github.com/martinspadrna/RaK/actions/runs/36027186627) SUCCESS a Vercel `dpl_J5XfjRw3oShK9NtFQL2ZDA3vhx34` READY; stabilní development alias na něj ukazuje. Metadata developmentu jsou jednotně `1.7.88`, SW cache `v1.7.88`, build `v1.7.88-calendar-admin-ui1`; release evidence má artefakt ID `10820885390` a rollback míří na READY `dpl_5zqBT2g5jDAZmJ9fNyxgdygznPn6`. `main` je `de443b771bb7e7dd5fefa498883fdd220a78f07d`; produkční Vercel zůstává `dpl_3Sn4PbVPMSAF2yrUTXKphoDEZ6tj` a produkční Supabase `bkqamcbkiwumsvelahxr`. Úplný fyzický checklist produkce 1.7.83 je potvrzený; fyzická přejímka aktuálního developmentu 1.7.88 zůstává otevřená. Vývojové práce pokračují pouze na `development`; další funkční změny nejprve do TEST `cgshssdjgzzuprlwnabl`.
...
**Stav 1.7.88:** implementováno a vydáno do TEST. Kalendáře mají vlastní administraci, A/B/C/D podporuje jeden i více Google Calendar odkazů, Dashboard používá směnu aktivního účtu a veřejný `.../public/basic.ics` se před uložením automaticky převádí na embed URL. Fyzický test 1.7.87 odhalil, že admin guard neznal režim `calendars`, takže Přidat i Uložit vracelo do Více a změna se neprovedla; 1.7.88 kořenově přidává `calendars` do povolených admin režimů. Současně jsou × a + Přidat na mobilu vedle sebe, nový řádek se vloží pod aktuální a mezi směnami je větší rozestup. CI/Chromium/TEST HTTP jsou PASS; fyzický iPhone retest ještě čeká. Procenta P2.2/P0.3 se zatím nemění, protože jejich otevřené checkboxy jsou širší než tento funkční balík.
...
7. **První nový TEST balík – základ implementován v 1.7.84 a nesen aktuálním 1.7.88, fyzická přejímka čeká:** local-first Dashboard/start, oddělení stavu účtů, account-scoped sync vzhledu s revizí/CAS a syntetický owner invariant jsou automaticky regresně ověřené. Před uzavřením potvrdit iPhone A→B / offline / dva přístroje / owner checklist; Chromium za něj nepovažovat.
8. **Druhý TEST balík – implementován v 1.7.86, public-ICS opraven v 1.7.87 a admin interakce/UI opraveny v 1.7.88, čeká fyzická přejímka:** směnové kalendáře jsou na preview `dpl_J5XfjRw3oShK9NtFQL2ZDA3vhx34`, Actions #285 PASS. Před uzavřením potvrdit na iPhonu, že Přidat/Uložit neopouští editor, × a + jsou vedle sebe, public ICS se skutečně uloží, A/B/C/D mapování funguje a private ICS je odmítnut.
