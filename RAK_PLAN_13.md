...
- Současný ověřený development runtime je `1.7.89`. Dokumentační nebo čistě testovací následník bez změny runtime číslo verze nezvyšuje.
...
- Poslední ověřený **funkční** development release: `a18c8e1be127702513f5fab40e43ad97f282a74f` (`1.7.89`). Živý HEAD se vždy zjišťuje online; případný následný dokumentační commit nesmí být zaměněn za nový runtime release.
...
- Stabilní development alias nyní ukazuje na `dpl_F8L2xx9ZzYF3hFtvfVyniouWard3`, READY / SHA `a18c8e1be127702513f5fab40e43ad97f282a74f`. Přesný SHA prošel [Actions #287](https://github.com/martinspadrna/RaK/actions/runs/36030461175) SUCCESS; release job přesunul alias až po celé fail-closed bráně. Ověřený rollback cíl je READY `dpl_J5XfjRw3oShK9NtFQL2ZDA3vhx34` / předchozí development 1.7.88.
- Release 1.7.89 prošel dvěma kanonickými buildy, celým `npm run check`, zděděnými i novými gate testy, rollback/ZIP důkazy, reálným Chromium online→offline→online testem, třemi PWA benchmarky, živou anonymní TEST HTTP kontrolou a ověřením nové přihlašovací RPC cesty. Release evidence `rak-release-evidence-a18c8e1be127702513f5fab40e43ad97f282a74f` má artefakt ID `10822310131`.
- Development metadata: viditelná `1.7.89`, technická `1.7.89`, modulová cache `1.7.89`, `package.json` `1.7.89`, SW cache `v1.7.89`, build `v1.7.89-calendar-account-modal1`. TEST Supabase zůstává `cgshssdjgzzuprlwnabl`; TEST migrace `20260924164526_rak_login_shift_team_v3_17089` přidala verzovanou `rak_lookup_account_for_login_v3`, která po již omezeném single-account login lookupu vrací pouze směnu právě nalezeného účtu A/B/C/D. Soukromý roster se běžnému klientovi nezpřístupnil. Profil směnu ukládá lokálně a starý profil bez ní se při prvním online startu doplní bez plošného mazání dat. Modal kalendáře nyní skutečně vyplňuje dostupnou výšku a zavírací × je centrované flex layoutem.
...
**RaK 1.7.89 – fyzická přejímka aktuálního developmentu ještě čeká.** Automatické Chromium/CI důkazy se za iPhone nevydávají. Release 1.7.85 opravil doručování PWA aktualizací a vlastník následně potvrdil, že se development skutečně aktualizoval na 1.7.85; tím je doložena update cesta, nikoli celý fyzický checklist local-first/account syncu.

Na stabilním development aliasu 1.7.89 je potřeba potvrdit zejména: (1) online návrat do instalované PWA zobrazí „kam jdu“ bez čekání na Supabase; (2) úplné zavření a start v režimu Letadlo zobrazí stejný lokální rozpis; (3) účet A → odhlásit → účet B ani krátce neukáže jméno nebo vzhled A; (4) stejný účet na dvou zařízeních převezme serverový vzhled a offline změna se po reconnectu deterministicky vyřeší bez globálního konfliktu; (5) přesun owner účtu mimo rozpis a zpět nezmění Auth/owner oprávnění; (6) Kalendáře: účet s explicitní směnou A/B/C musí otevřít svou směnu i bez přístupu k privátnímu rosteru, veřejný `public/basic.ics` musí zobrazit správný kalendář, iframe má využít většinu výšky modalu a zavírací × má být opticky uprostřed kruhu; administrátorské Přidat/Odebrat/Načíst/Uložit nadále nesmí opustit editor. Fyzický test 1.7.88 doložil konkrétní regresi: účet nastavený na A zobrazil fallback D, iframe byl jen malý proužek a × nebylo vycentrované; 1.7.89 tyto tři příčiny opravuje a čeká na retest. Do tohoto potvrzení se žádný iPhone-dependent checkbox nezavírá.
...
**Aktuální online stav k 24. 9. 2026:** produkční předání 1.7.83 včetně schválené bezpečnostní fáze B je dokončeno a produkce zůstává beze změny. Nejnovější TEST runtime je 1.7.89 na funkčním SHA `a18c8e1be127702513f5fab40e43ad97f282a74f`, [Actions #287](https://github.com/martinspadrna/RaK/actions/runs/36030461175) SUCCESS a Vercel `dpl_F8L2xx9ZzYF3hFtvfVyniouWard3` READY; stabilní development alias na něj ukazuje. Metadata developmentu jsou jednotně `1.7.89`, SW cache `v1.7.89`, build `v1.7.89-calendar-account-modal1`; release evidence má artefakt ID `10822310131` a rollback míří na READY `dpl_J5XfjRw3oShK9NtFQL2ZDA3vhx34`. TEST Supabase má navíc migraci `20260924164526_rak_login_shift_team_v3_17089`; produkční databáze ji nedostala. `main` je `de443b771bb7e7dd5fefa498883fdd220a78f07d`; produkční Vercel zůstává `dpl_3Sn4PbVPMSAF2yrUTXKphoDEZ6tj` a produkční Supabase `bkqamcbkiwumsvelahxr`. Úplný fyzický checklist produkce 1.7.83 je potvrzený; fyzická přejímka aktuálního developmentu 1.7.89 zůstává otevřená. Vývojové práce pokračují pouze na `development`; další funkční změny nejprve do TEST `cgshssdjgzzuprlwnabl`.
...
**Stav 1.7.89:** implementováno a vydáno do TEST. Administrace, public-ICS normalizace a oprava admin guardu z 1.7.86–1.7.88 zůstávají zachované. Fyzický test 1.7.88 odhalil tři další konkrétní chyby: účet s explicitní směnou A zobrazil fallback D, protože běžný klient záměrně nevidí privátní worker roster; Google iframe zůstal malý, protože jeho `.calendarModalFrameWrap` neměl flex rodiče; zavírací × nebylo opticky centrované. 1.7.89 přidává bezpečný bounded login RPC v3, který vrací pouze směnu právě nalezeného účtu a profil ji ukládá i pro offline/restart, bez zveřejnění seznamu pracovníků. Modal má skutečný flex obsah přes zbývající výšku a × používá explicitní flex centrování. CI/Chromium/TEST HTTP a anonymní TEST RPC ověření jsou PASS; fyzický iPhone retest ještě čeká. Procenta P2.2/P0.3 se zatím nemění, protože jejich otevřené checkboxy jsou širší než tento funkční balík.
...
7. **První nový TEST balík – základ implementován v 1.7.84 a nesen aktuálním 1.7.89, fyzická přejímka čeká:** local-first Dashboard/start, oddělení stavu účtů, account-scoped sync vzhledu s revizí/CAS a syntetický owner invariant jsou automaticky regresně ověřené. Před uzavřením potvrdit iPhone A→B / offline / dva přístroje / owner checklist; Chromium za něj nepovažovat.
8. **Druhý TEST balík – implementován v 1.7.86, public-ICS opraven v 1.7.87, admin interakce/UI v 1.7.88 a mapování účtu + modal v 1.7.89, čeká fyzická přejímka:** směnové kalendáře jsou na preview `dpl_F8L2xx9ZzYF3hFtvfVyniouWard3`, Actions #287 PASS. Před uzavřením potvrdit na iPhonu, že účet A/B/C/D otevře správnou směnu bez privátního rosteru, public ICS skutečně zobrazí správný kalendář, iframe využije většinu výšky modalu, × je vycentrované a administrátorské Přidat/Uložit neopouští editor.
