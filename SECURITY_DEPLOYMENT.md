...
## Následný audit starších preview a TEST záloh – 2026-09-23

Read-only kontrola proběhla po bezpečnostním balíku na development HEAD `3c040643cc01e27bb95de37a1ca5e0735ca86fad`. Čtyři READY preview deploymenty byly načteny přes autentizovaný Vercel CLI přístup bez shareable parametru a bez výpisu API odpovědních těl:

- `dpl_Hci5FbQCWFcehJTkfHkWCNGvM3ya` / `3c040643`: TEST ano, produkční Supabase ne, oba vyřazené API endpointy HTTP 410.
- `dpl_C6fkkvMNdV9LSqcV9eHnxdi33dzX` / `cd6e8b89`: TEST ano, produkční Supabase ne, oba vyřazené API endpointy HTTP 410.
- `dpl_Erg2nVhGsQtdgpRk81wBz7A9tPcy` / `445f6d18`: TEST ano, produkční Supabase ne, oba vyřazené API endpointy HTTP 410.
- `dpl_FKtZmysaTpjWibxTAzRyVP1shzkw` / `173d57fd`: TEST ano, produkční Supabase ne, oba vyřazené API endpointy HTTP 410.
